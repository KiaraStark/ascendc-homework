...
BIN_PATH="$OUTPUT_DIR/execute_softmax_aclnn"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            cat <<'USAGE'
Usage: ./run.sh [--dtype fp32|fp16] [--rebuild]

Options:
  --dtype   Data type used by data generation and verification. Default: fp32
...
if [[ "$DTYPE" != "fp32" && "$DTYPE" != "fp16" ]]; then
    echo "[ERROR] --dtype must be fp32 or fp16"
    exit 1
fi
