...
bool OpRunner::RunOp()
{
    if (numInputs_ != 1 || numOutputs_ != 3) {
        ERROR_LOG("SoftmaxCustom expects 1 input and 3 outputs, got %zu inputs, %zu outputs", numInputs_, numOutputs_);
        return false;
    }
    ...
    aclnnStatus ret = aclnnSoftmaxCustomGetWorkspaceSize(
        inputTensor_[0], outputTensor_[0], outputTensor_[1], outputTensor_[2], &workspaceSize, &executor);
    if (ret != ACL_SUCCESS) {
        ERROR_LOG("aclnnSoftmaxCustomGetWorkspaceSize failed, ret=%d", static_cast<int>(ret));
    ...
    ret = aclnnSoftmaxCustom(workspace, workspaceSize, executor, stream);
    if (ret != ACL_SUCCESS) {
        ERROR_LOG("aclnnSoftmaxCustom failed, ret=%d", static_cast<int>(ret));
...
template<typename T>
static void DoPrintData(const T *data, size_t count, size_t elementsPerRow)
{
...
static void DoPrintFp16Data(const aclFloat16 *data, size_t count, size_t elementsPerRow)
{
    assert(elementsPerRow != 0);
    for (size_t i = 0; i < count; ++i) {
...
static void PrintData(const void *data, size_t count, aclDataType dataType, size_t elementsPerRow)
{
    if (data == nullptr) {
    ...
    switch (dataType) {
        ...
        case ACL_FLOAT:
            ...
            break;
        default:
            WARN_LOG("Unsupported dtype in PrintData: %d", static_cast<int>(dataType));
            break;
    }
}

void OpRunner::PrintInput(size_t index, size_t elementsPerRow)
{
    if (index >= numInputs_) {
        ERROR_LOG("index out of range. index = %zu, numInputs = %zu", index, numInputs_);
        return;
    }
    PrintData(hostInputs_[index], GetInputElementCount(index), GetInputDataType(index), elementsPerRow);
}

void OpRunner::PrintOutput(size_t index, size_t elementsPerRow)
{
    if (index >= numOutputs_) {
        ERROR_LOG("index out of range. index = %zu, numOutputs = %zu", index, numOutputs_);
        return;
    }
    PrintData(hostOutputs_[index], GetOutputElementCount(index), GetOutputDataType(index), elementsPerRow);
}
