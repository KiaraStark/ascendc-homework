...
def _build_args(x_in__, max_out_, sum_out_, z_out_):
    __inputs__ = []
    for arg in [x_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [max_out_, sum_out_, z_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("SoftmaxCustom", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def softmax_custom(x_in__, max_out_, sum_out_, z_out_, kernel_name="softmax_custom", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, max_out_, sum_out_, z_out_)
    ...
    origin_func_name = "softmax_custom"
    ascendc_src_dir_ex = "softmax_custom"
    ascendc_src_dir = "softmax_custom"
    ...
    if src_file != "":
        ...
        ascendc_src_file = "softmax_custom.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator SoftmaxCustom, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "SoftmaxCustom"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[x_in__], origin_outputs = [max_out_, sum_out_, z_out_],\
...
def op_select_format(x_in__, max_out_, sum_out_, z_out_, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, max_out_, sum_out_, z_out_)
    result = check_op_cap("op_select_format", "SoftmaxCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, max_out_, sum_out_, z_out_, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, max_out_, sum_out_, z_out_)
    result = check_op_cap("get_op_specific_info", "SoftmaxCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
