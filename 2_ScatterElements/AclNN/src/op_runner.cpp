...
{
    if (numInputs_ != 3) {
        ERROR_LOG("ScatterElements expects 3 inputs, got %zu", numInputs_);
        return false;
    }
    ...
    auto ret = aclnnScatterElementsGetWorkspaceSize(
        inputTensor_[0], inputTensor_[1], inputTensor_[2], opDesc_->axis, opDesc_->reduce, &workspaceSize, &executor);
    if (ret != ACL_SUCCESS) {
        ERROR_LOG("aclnnScatterElementsGetWorkspaceSize failed. ret=%d", static_cast<int32_t>(ret));
    ...
    ret = aclnnScatterElements(workspace, workspaceSize, executor, stream);
    if (ret != ACL_SUCCESS) {
        ERROR_LOG("aclnnScatterElements failed. ret=%d", static_cast<int32_t>(ret));
    ...
    // ScatterElements updates var in-place, write back input 0 for result checking.
    {
        const size_t size = GetInputSize(0);
        aclrtMemcpyKind kind = g_isDevice ? ACL_MEMCPY_DEVICE_TO_DEVICE : ACL_MEMCPY_DEVICE_TO_HOST;
        if (aclrtMemcpy(hostInputs_[0], size, devInputs_[0], size, kind) != ACL_SUCCESS) {
            ERROR_LOG("copy in-place result to host failed");
            if (workspace != nullptr) {
                (void)aclrtFree(workspace);
            }
            (void)aclrtDestroyStream(stream);
