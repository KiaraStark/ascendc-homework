...
#include "aclnn_scatter_elements.h"
...
#include "ScatterElements_op_resource.h"
...
[[maybe_unused]] uint32_t NNOPBASE_ScatterElements = 0U;
} // namespace
    ...
    aclOpExecutor **executor)
{
...
#ifdef ACLNN_WITH_BINARY
    static uint32_t ScatterElementsOpTypeId = op::GenOpTypeId("ScatterElements", ScatterElements_RESOURCES);
#endif
    static NnopbaseDfxId dfxId = {0x60000, __func__, false};
    static NnopbaseDfxId tilingId = {0x60000, "aclnnScatterElementsTiling", false};
    void *nnopExecutor;
    static void *executorSpace = NULL;
    const char *opType = "ScatterElements";
    char inputDesc[] = {1, 1, 1};
    char outputDesc[] = {1};
    char attrDesc[] = {0, 0};

    NNOPBASE_ASSERT_NOTNULL_RETVAL(varRef);
    NNOPBASE_ASSERT_NOTNULL_RETVAL(indices);
    NNOPBASE_ASSERT_NOTNULL_RETVAL(updates);
    ...
    *executor = reinterpret_cast<aclOpExecutor *>(nnopExecutor);
...
#ifdef ACLNN_WITH_BINARY
    NnopbaseAddOpTypeId(*executor, ScatterElementsOpTypeId);
#endif
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseSetRef(*executor, 0, 0));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, varRef, 0));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, indices, 1));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, updates, 2));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(&axis), sizeof(int64_t), 0, kNnopbaseInt));
    if (reduceOptional) {
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(reduceOptional), strlen(reduceOptional) + 1, 1, kNnopbaseString));
    } else {
        static char *reduceOptionalDef = "None";
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(reduceOptionalDef), strlen(reduceOptionalDef) + 1, 1, kNnopbaseString));
    }
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddOutput(*executor, varRef, 0));
    ...
    if (NnopbaseAddParamName != NULL) {
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 0, "varRef", true));
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 1, "indices", true));
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 2, "updates", true));
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 0, "varRef", false));
    }
