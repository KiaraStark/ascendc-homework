...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['var', 'indices', 'updates']
    output_names = ['var']
...
def _build_args(var_in__, indices_in__, updates_in__, var_out_, axis, reduce):
    __inputs__ = []
    for arg in [var_in__, indices_in__, updates_in__]:
        if arg != None:
...
@tbe_register.register_operator("ScatterElements", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_STR, para_check.KERNEL_NAME)
def scatter_elements(var_in__, indices_in__, updates_in__, var_out_, axis=0, reduce="None", kernel_name="scatter_elements", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, indices_in__, updates_in__, var_out_, axis, reduce)
    ...
    origin_func_name = "scatter_elements"
    ascendc_src_dir_ex = "scatter_elements"
    ascendc_src_dir = "scatter_elements"
    ...
    if src_file != "":
        ...
        ascendc_src_file = "scatter_elements.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator ScatterElements, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[var_in__, indices_in__, updates_in__], origin_outputs = [var_out_],\
...
def op_select_format(var_in__, indices_in__, updates_in__, var_out_, axis=0, reduce="None", impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, indices_in__, updates_in__, var_out_, axis, reduce)
    result = check_op_cap("op_select_format", "ScatterElements", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(var_in__, indices_in__, updates_in__, var_out_, axis=0, reduce="None", impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, indices_in__, updates_in__, var_out_, axis, reduce)
    result = check_op_cap("get_op_specific_info", "ScatterElements", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
