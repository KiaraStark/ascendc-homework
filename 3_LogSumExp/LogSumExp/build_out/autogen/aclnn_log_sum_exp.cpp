...
#include "LogSumExp_op_resource.h"
...
uint32_t socSupportList[] = {SOC_VERSION_ASCEND310B,SOC_VERSION_ASCEND910B};
...
[[maybe_unused]] uint32_t NNOPBASE_LogSumExp = 0U;
} // namespace
...
#ifdef ACLNN_WITH_BINARY
    static uint32_t LogSumExpOpTypeId = op::GenOpTypeId("LogSumExp", LogSumExp_RESOURCES);
#endif
    static NnopbaseDfxId dfxId = {0x60000, __func__, false};
    static NnopbaseDfxId tilingId = {0x60000, "aclnnLogSumExpTiling", false};
    void *nnopExecutor;
    static void *executorSpace = NULL;
    const char *opType = "LogSumExp";
    char inputDesc[] = {1};
    char outputDesc[] = {1};
    char attrDesc[] = {0, 0};
...
#ifdef ACLNN_WITH_BINARY
    NnopbaseAddOpTypeId(*executor, LogSumExpOpTypeId);
#endif
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, x, 0));
    if (dimOptional) {
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddIntArrayAttr(*executor, dimOptional, 0));
    } else {
        static int64_t dimOptionalDef[] = {};
        static size_t dimOptionalLen = 1;
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddArrayAttrWithDtype(*executor, static_cast<void*>(dimOptionalDef), dimOptionalLen, sizeof(int64_t), 0, kNnopbaseInt));
    }
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(&keepDim), sizeof(bool), 1, kNnopbaseBool));
