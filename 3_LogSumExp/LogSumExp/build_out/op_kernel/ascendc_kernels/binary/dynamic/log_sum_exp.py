...
def _build_args(x_in__, y_out_, dim, keep_dim):
    __inputs__ = []
    for arg in [x_in__]:
        if arg != None:
...
@tbe_register.register_operator("LogSumExp", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def log_sum_exp(x_in__, y_out_, dim=[], keep_dim=False, kernel_name="log_sum_exp", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, dim, keep_dim)
    ...
    origin_func_name = "log_sum_exp"
    ascendc_src_dir_ex = "log_sum_exp"
    ascendc_src_dir = "log_sum_exp"
    ...
    if src_file != "":
        ...
        ascendc_src_file = "log_sum_exp.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator LogSumExp, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "LogSumExp"
...
def op_select_format(x_in__, y_out_, dim=[], keep_dim=False, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, dim, keep_dim)
    result = check_op_cap("op_select_format", "LogSumExp", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_out_, dim=[], keep_dim=False, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, dim, keep_dim)
    result = check_op_cap("get_op_specific_info", "LogSumExp", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
