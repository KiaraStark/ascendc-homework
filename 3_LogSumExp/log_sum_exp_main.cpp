...
{
    // Match 3_LogSumExp/gen_logsumexp_data.py defaults.
    ...
    OperatorDesc opDesc;
    opDesc.dim = {0};
    opDesc.keepdim = false;
    opDesc.AddInputTensorDesc(dataType, shape.size(), shape.data(), format);
    opDesc.AddOutputTensorDesc(dataType, outShape.size(), outShape.data(), format);
    return opDesc;
}
...
{
    size_t fileSize = 0;
    ReadFile("./3_LogSumExp/input/input_x.bin", fileSize, runner.GetInputBuffer<void>(0), runner.GetInputSize(0));
...
{
    WriteFile("./3_LogSumExp/output/output.bin", runner.GetOutputBuffer<void>(0), runner.GetOutputSize(0));
...
bool InitResource()
{
    std::string output = "./3_LogSumExp/output";
