"""
Softmax Operator Tests with AscendC Kernel
"""
...
# Test the numpy-based softmax implementation
...
def test_kernel_compilation():
    """测试kernel是否成功编译"""
    kernel_lib_path = os.path.join(os.path.dirname(__file__), 'libsoftmax_kernel.a')
    assert os.path.exists(kernel_lib_path), f"Kernel library not found at {kernel_lib_path}"
    print("test_kernel_compilation PASSED")
...
def test_kernel_load():
    ...
    try:
        ...
        print("test_kernel_load PASSED")
        return True
    except Exception as e:
        print(f"test_kernel_load FAILED: {e}")
        return False
